...
def compute_value_area(
    ...
    va_mid_from_avg_poc: bool = False
) -> Dict[str, Union[float, str, None]]:
    if not volume_profile:
        return {'vah': None, 'val': None, 'va_mid': None, 'vah_bucket': None, 'val_bucket': None}
    ...
    if local_maxima_buckets:
        if len(local_maxima_buckets) > 1 and va_mid_from_avg_poc:
            poc_prices = []
            for b in local_maxima_buckets:
                if '-' in b['bucket_key']:
                    low, high = map(float, b['bucket_key'].split(' - '))
                    poc_prices.append((low + high) / 2)
                else:
    ...
    try:
        ...
        return {'vah': None, 'val': None, 'va_mid': poc_bucket_key, 'vah_bucket': None, 'val_bucket': None}
    ...
    val_bucket = None
    if vah is not None:
        for b in included_buckets:
            if b['max'] == vah:
                vah_bucket = b['bucket_key']
                break
    if val is not None:
        for b in included_buckets:
            if b['min'] == val:
                val_bucket = b['bucket_key']
                break
    return {
        'vah': vah,
        'val': val,
        'va_mid': poc_bucket_key,
        'vah_bucket': vah_bucket,
...
def compute_candles_stats(
    ...
    distance_from_ema_bps_buckets : Dict[
                ...
                Dict[str,Union[float, List[float]]]
                                                    ...
                                                    values = [ -2000, -1500, -1000, -750, -500, -400, -300, -200, -150, -100, -75, -50, -25, 0, 25, 50, 100, 150, 200, 300, 400, 500, 750, 1000, 1500, 2000 ], # pd_candles['distance_from_ema_bps'].tolist(),
                                                    outlier_threshold_percent = 10,
                                                    level_granularity=level_granularity
            )
