...
def parse_args():
    ...
    parser.add_argument("--max_slippage_percent_tp_min", help="max_slippage_percent_tp_min is for max slippage control at the point when order is dispatched to gateway and gateway executing the first slice. It's enforced on gateway side. This is for ENTRIES only. For EXITS, generally you just want to get out. 20% max_slippage_percent_tp_min means 20% on tp_min_percent. If tp_min_percent is 1%, then max tolerance is 1% x 20% = 0.2% slippage from ref_price (i.e. mid at the time when orders dispatched to gateway). Default: None (i.e. Not enforced).", default=None)
    ...
    param['max_slippage_percent_tp_min'] = float(args.max_slippage_percent_tp_min) if args.max_slippage_percent_tp_min else None
...
async def main():
    ...
    hi_candle_size : str = hi_candles_w_ta_topic.split('-')[-3]
    lo_candle_size : str = lo_candles_w_ta_topic.split('-')[-3]
    ...
    if exchange:
        ...
        dispatch_notification(title=f"{param['current_filename']} {param['gateway_id']} strategy {TargetStrategy.__name__} starting", message=balances['total'] if not param['privacy_first'] else f"{param['gateway_id']} started", footer=param['notification']['footer'], params=notification_params, log_level=LogLevel.CRITICAL, logger=logger)
            ...
            try:
                ...
                if param['trading_window_start'] and param['trading_window_end']:
                    ...
                    if in_window!=parsed_trading_window['in_window']:
                        ...
                        dispatch_notification(title=f"{param['current_filename']} {param['gateway_id']} {_ticker} {trading_window_notification_msg}.", message=trading_window, footer=param['notification']['footer'], params=notification_params, log_level=LogLevel.CRITICAL, logger=logger)
                ...
                '''
                if not columns_type_initialized:
                    columns_type_initialized = True # Do this only once
                    
                    pd_position_cache['created'] = pd_position_cache['created'].astype(object)
                ...
                if any_target_adj and pos_usdt!=0:
                    any_target_adj = False
                    
                    tp_max_pnl_est = abs(pos_usdt) * tp_max_percent/100
                    tp_min_pnl_est = abs(pos_usdt) * tp_min_percent/100
                    ...
                    if not pos or pos==0: # @todo: align with backtest_core, allow multi-slices entries
                        ...
                        if allow_entry and not block_entries:
                            ...
                            if allow_entry_final_long or allow_entry_final_short:
                                ...
                                dispatch_notification(title=f"#entry {param['current_filename']} {gateway_id} Entry succeeded. {_ticker} {side} {param['amount_base_ccy'] if not param['privacy_first'] else '---'} (USD amount: {amount_filled_usdt if not param['privacy_first'] else '---'}) @ {entry_px}", message=executed_position['position'], footer=param['notification']['footer'], params=notification_params, log_level=LogLevel.CRITICAL, logger=logger)
