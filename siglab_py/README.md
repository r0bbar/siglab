...
+ Multiple **DivisiblePosition** executed in **Parallel**, while slices are executed **Sequentially**. Atm, there's no synchronization if you're executing multiple positions. So, one position may have executed 3/10 slices. The second position may have already executed 6/10 slices. Also we haven't added random delay in slice execution, if some of your want to make it less obvious this is bot execution? 
