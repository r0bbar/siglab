            ...
            for new_headline in new_headlines:
                try:
                    ...
                    if loop_counter>0 and (any([ keyword for keyword in param['focus_keywords'] if keyword in new_headline['title'] ])): # Don't send flood of slacks in first iteration
                        dispatch_notification(title=f"[{param['current_filename']}] New headline arrived", message=new_headline, footer=param['notification']['footer'], params=notification_params, log_level=LogLevel.CRITICAL, logger=logger)
